...
function getOptionGroups(product) {
  ...
  possible.forEach(group => {
    if (Array.isArray(product[group])) {
      groups[group] = product[group];
    } else if (
      Array.isArray(product.price_options?.[group])
    ) {
      groups[group] =
        product.price_options[group];
    } else {
      groups[group] = [];
    }
  });
...
function getMaxProductPrice(product) {
  ...
  Object.values(groups).forEach(list => {
    ...
    const max = list.reduce(
      (result, option) => {
        ...
        return Math.max(result, surcharge);
      },
      0
    );
  ...
  if (maxSurcharge > 0) {
    return base + maxSurcharge;
  }

  const variants = getLegacyVariants(product);
...
function resetProductForm() {
  ...
  $("#productMemories").innerHTML = "";
  $("#productSims").innerHTML = "";
  $("#productRegions").innerHTML = "";
...
function addEmptyOptionPlaceholder(group) {
  const container = getOptionContainer(group);
...
function addOptionRow(
  ...
  container
    .querySelector("[data-option-empty]")
    ?.remove();
  ...
  row
    .querySelector("[data-delete-option]")
    ?.addEventListener(
      "click",
      () => {
        row.remove();
  ...
  row
    .querySelector("[data-option-surcharge]")
    ?.addEventListener(
      "input",
      updatePricePreview
    );
...
function renumberOptions(group) {
  ...
  [...container.children]
    .filter(
      element =>
        element.classList.contains("option-row")
    )
    .forEach((row, index) => {
      const numberElement =
        row.querySelector(".option-number");
...
function readOptions(group) {
  ...
  return [...container.querySelectorAll(".option-row")]
    .map(row => {
      ...
      const name =
        row
          .querySelector("[data-option-name]")
          ?.value
          ?.trim() || "";
...
async function saveBanner(event) {
  ...
  const payload = {
    ...
    product_id:
      $("#bannerProduct").value || null,

    sort_order:
      number($("#bannerSortOrder").value),

    active:
      number($("#bannerActive").value)
  };
...
async function saveReservationAmount() {
  const amount =
    number($("#reservationAmount").value);
  ...
  try {
    ...
    toast("Сумма бронирования сохранена");
...
async function saveReservationCard(event) {
  ...
  const id =
    $("#reservationCardId").value.trim();

  const payload = {
    card_number:
      $("#reservationCardNumber").value.trim(),

    recipient:
      $("#reservationRecipient").value.trim(),

    bank:
      $("#reservationBank").value.trim(),
...
function resetPickupForm() {
  ...
  $("#pickupSortOrder").value = "0";
  $("#pickupActive").value = "1";
...
async function savePickup(event) {
  ...
  const payload = {
    ...
    sort_order:
      number($("#pickupSortOrder").value),

    active:
      number($("#pickupActive").value)
  };
...
async function quickUpdateOrderStatus(
  ...
  try {
    ...
    if (
      currentOrderId &&
      String(currentOrderId) === String(id)
    ) {
...
async function openOrderDetails(id) {
  ...
  try {
    ...
    try {
      ...
      // Если отдельный detail endpoint
      // недоступен, используем уже загруженный заказ.
    }
