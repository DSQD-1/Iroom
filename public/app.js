...
(() => {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      let fullOrder = firstCreated;

      try {
        const orderData = await api(
          `/api/orders/${encodeURIComponent(firstCreated.id)}`
        );
        ...
        console.warn("ORDER DETAIL LOAD:", error);
      }
        ...
        <div class="order-card" style="padding:20px;margin-bottom:12px">
          ...
          <div class="summary" style="margin-top:16px">
            ...
            <button
              id="profilePhoneRow"
              type="button"
              style="width:100%;border:0;background:transparent;color:inherit;padding:0;text-align:left;cursor:pointer;font:inherit"
            >
              <div class="summary-row" style="margin:0">
                <span>Телефон</span>
                <strong>${escapeHtml(user.phone || "Не добавлен")} <span style="color:#ff2b8a;margin-left:4px">›</span></strong>
              </div>
            </button>
          </div>

          <div id="profilePhoneEditor" style="display:none;margin-top:14px;padding:12px;border-radius:16px;background:#151515">
            <div style="font-size:13px;color:#999;margin-bottom:8px">Номер телефона</div>
            <div style="display:flex;gap:8px">
              <input id="profilePhone" class="checkout-input" type="tel" inputmode="tel" autocomplete="tel" placeholder="+7 999 123-45-67" value="${escapeHtml(user.phone || "")}" style="flex:1">
              <button class="secondary-button" id="saveProfilePhone" type="button" style="white-space:nowrap">Сохранить</button>
            </div>
          </div>
        </div>
  ...
  async function saveProfilePhone() {
    ...
    try {
      const data = await api("/api/me", { method: "PUT", body: { phone } });
