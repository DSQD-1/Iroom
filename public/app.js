...
function isPromoCodeActive(value) {
  ...
  return ACTIVE_PROMO_CODES.includes(code);
}
...
function setupNavigation() {
  ...
  document
    .querySelector('[data-category-id="all"]')
...
function renderProductDetails() {
  ...
  document
    .getElementById("bookProductButton")
    ?.addEventListener(
      "click",
      openCheckout
    );
  ...
  document
    .getElementById("productContactButton")
    ?.addEventListener(
      "click",
      () => {
        openContact(
  ...
  container.innerHTML = `
    ...
    <div class="checkout-card reservation-box">
      ...
      <div class="checkout-note">
        ...
        внести 1 000 ₽. Реквизиты предоставляет
        магазин.
      </div>
...
function renderCheckoutAddress(type) {
  ...
  container.innerHTML = `
    <div class="checkout-address-info">
      ...
      <br>
      Точный адрес и время работы
      подтверждает менеджер после оформления
      заказа.
    </div>
  `;
}
...
async function createOrder() {
  ...
  try {
    ...
    if (enteredPromoCode) {
      if (!isPromoCodeActive(enteredPromoCode)) {
        throw new Error(
          "Промокод не найден или больше не действует"
        );
      }
...
async function safeLoadStore() {
  /*
    ВАЖНО:
    Каждый API-запрос выполняется отдельно.
    Ошибка одного запроса не блокирует весь магазин.
  */
  ...
  /*
    Заказы НЕ блокируют запуск магазина.
  */
  loadOrders().catch(
    (error) => {
      console.error(
...
document.addEventListener(
  ...
  async () => {
    try {
      ...
      /*
        Сразу показываем магазин.
        Никакого ожидания API перед открытием.
      */
      hideStartupScreen();

      /*
        Данные догружаются после открытия.
      */
      await safeLoadStore();
    } catch (error) {
      console.error(
      ...
      /*
        ...
        интерфейс всё равно открывается.
      */
      hideStartupScreen();
