...
(() => {
  ...
  function loadViewedProducts() {
    try {
      const raw = localStorage.getItem("iroom_viewed_products");
      const ids = JSON.parse(raw || "[]");
      state.viewedProducts = Array.isArray(ids) ? ids.map(Number).filter(Number.isFinite) : [];
    } catch { state.viewedProducts = []; }
  }

  function saveViewedProducts() {
    try { localStorage.setItem("iroom_viewed_products", JSON.stringify(state.viewedProducts.slice(0, 12))); } catch {}
  }

  function rememberViewedProduct(product) {
    const id = Number(product?.id);
    if (!id) return;
    state.viewedProducts = [id, ...state.viewedProducts.filter((value) => value !== id)].slice(0, 12);
    saveViewedProducts();
  }
  ...
  async function openProduct(
    ...
    try {
      ...
      rememberViewedProduct(data.product);
  ...
  async function placeOrder() {
    ...
    if (!state.checkout.contact.trim()) {
      state.checkout.step = 1;
      render();
    ...
    if (state.checkout.fulfillment === "delivery") {
      if (
        ...
        render();
        toast("Укажите город, улицу и дом");
        return;
      }
    }
    ...
    try {
      ...
      for (const item of state.cart) {
        const payload = {
          ...
          fulfillment_type: state.checkout.fulfillment,
          receiving_type: state.checkout.fulfillment,
          delivery_type: state.checkout.fulfillment,
          pickup_point_id:
            state.checkout.fulfillment === "pickup"
              ? state.checkout.pickupPointId
              : null,
          delivery_city: state.checkout.city,
          delivery_street: state.checkout.street,
          delivery_house: state.checkout.house,
          delivery_apartment: state.checkout.apartment,
          customer_comment: [
      ...
      if (!created.length || !created[0]?.id) {
        throw new Error("Сервер не вернул созданный заказ");
      }

      /*
       * Пользователь нажал именно «Я внёс залог».
       * Поэтому после создания заказа сразу отправляем
       * заявку на ручную проверку залога.
       * Сервер сам уведомит администраторов в Telegram.
       */
      const paidOrders = [];

      for (const createdOrder of created) {
        if (num(createdOrder.reservation_amount) > 0) {
          const paidData = await api(
            `/api/orders/${encodeURIComponent(createdOrder.id)}/reservation-paid`,
            { method: "POST" }
          );

          if (paidData?.order) {
            paidOrders.push(paidData.order);
          }
        } else {
          paidOrders.push(createdOrder);
        }
      }

      // Сервер уже возвращает полный заказ после reservation-paid.
      // Не делаем второй GET сразу после создания: на Turso это могло
      // давать кратковременный 404 при чтении только что созданной записи.
      const firstCreated = paidOrders[0] || created[0];
      const fullOrder = firstCreated;
      ...
      state.checkout = {
        ...
        comment: "",
        fulfillment: "pickup",
        pickupPointId: state.pickupPoints[0]?.id || null,
        city: "",
        street: "",
        house: "",
        apartment: "",
      ...
      const existingIndex = state.orders.findIndex(
        order => String(order.id) === String(fullOrder.id)
      );

      if (existingIndex >= 0) {
        state.orders[existingIndex] = fullOrder;
      } else {
        state.orders.unshift(fullOrder);
      }

      /* После внесения залога отправляем пользователя в профиль. */
      try { await loadMe(); } catch {}
      state.route = "profile";
      render();

      haptic("success");
      toast("Заказ создан. Залог отправлен на проверку");
    } catch (error) {
      console.error("PLACE ORDER:", error);

      if (button) {
        button.disabled = false;
        button.textContent = "Я внёс залог";
      }
  ...
  function profileInitials(user) {
    const text = String(user?.first_name || user?.username || "IR").trim();
    return escapeHtml(text.slice(0, 1).toUpperCase() || "I");
  }

  function profileTelegramLabel(user) {
    if (!user) return "Telegram";
    if (user.username) return `@${user.username}`;
    return [user.first_name, user.last_name].filter(Boolean).join(" ") || "Telegram";
  }
    ...
    screen.innerHTML = `
      <div class="screen">
        <div class="page-head">
          ...
          <h1>Профиль</h1>
          <p>Ваши данные и покупки в IRoom</p>
        </div>

        <div class="order-card" style="padding:20px;margin-bottom:12px">
          <div style="display:flex;align-items:center;gap:14px">
            <div style="width:54px;height:54px;border-radius:18px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#ff2b8a,#ff5cad);color:#fff;font-size:24px;font-weight:800">
              ${profileInitials(user)}
            </div>
            <div style="min-width:0">
              <div style="font-size:20px;font-weight:800">
                ${escapeHtml([user.first_name,user.last_name].filter(Boolean).join(" ") || "Пользователь Telegram")}
              </div>
              <div style="color:#999;margin-top:4px">
                ${escapeHtml(profileTelegramLabel(user))}
              </div>
            </div>
          </div>

          <div class="summary" style="margin-top:16px">
            <div class="summary-row"><span>Telegram</span><strong>${escapeHtml(profileTelegramLabel(user))}</strong></div>
            <button id="profilePhoneRow" type="button" class="summary-row" style="width:100%;border:0;background:transparent;color:inherit;text-align:left;padding:12px 0;cursor:pointer">
              <span>Телефон</span><strong>${escapeHtml(user.phone || "Добавить →")}</strong>
            </button>
          </div>

          <div id="profilePhoneEditor" style="display:none;gap:8px;margin-top:8px">
            <input id="profilePhone" class="checkout-input" type="tel" inputmode="tel" autocomplete="tel" placeholder="+7 900 000-00-00" value="${escapeHtml(user.phone || "")}" style="flex:1">
            <button class="primary-button" id="saveProfilePhone" type="button" style="white-space:nowrap;padding:0 18px">Сохранить</button>
          </div>
        </div>

        <div class="order-card" style="padding:18px;margin-bottom:12px">
          <div style="font-size:18px;font-weight:800;margin-bottom:12px">Заказы</div>
          <button class="primary-button" data-action="orders" type="button" style="width:100%">Мои заказы ${state.orders.length ? `· ${state.orders.length}` : ""}</button>
        </div>

        <div class="order-card" style="padding:18px;margin-bottom:12px">
          <div style="font-size:18px;font-weight:800;margin-bottom:4px">Недавно смотрели</div>
          <div style="color:#888;font-size:13px;margin-bottom:14px">Последние товары, которые вы открывали</div>
          ${recent.length ? `<div class="products-grid">${recent.map(productCard).join("")}</div>` : `<div class="empty" style="padding:20px 0"><strong>Пока пусто</strong><p>Откройте товар в каталоге — он появится здесь.</p></div>`}
        </div>

        <div class="order-card" style="padding:18px">
          <div style="font-size:18px;font-weight:800;margin-bottom:12px">Ещё</div>
          <button class="secondary-button" data-action="catalog" type="button" style="width:100%;margin-bottom:8px">Каталог</button>
          <button class="secondary-button" data-action="cart" type="button" style="width:100%">Корзина</button>
        </div>
      </div>
    `;

    $("#profilePhoneRow")?.addEventListener("click", () => {
      const editor = $("#profilePhoneEditor");
      if (editor) {
        editor.style.display = editor.style.display === "none" ? "flex" : "none";
        if (editor.style.display !== "none") setTimeout(() => $("#profilePhone")?.focus(), 50);
      }
    });
    $("#saveProfilePhone")?.addEventListener("click", saveProfilePhone);
    bindDynamicEvents();
  }
  ...
  async function saveProfilePhone() {
    ...
    try {
      const data = await api("/api/me/phone", { method: "POST", body: { phone } });
      state.me = data?.user || state.me;
      await loadMe();
      renderProfile($("#screen"));
      toast("Номер сохранён");
      haptic("success");
    } catch (error) { toast(error.message || "Не удалось сохранить номер"); }
  }
              ...
              ? `
                <div class="info-box">
                  ...
                  <div style="margin-top:6px">
                    🟠 Ожидайте проверки залога
                  </div>
                  <div style="margin-top:6px">
                    Мы получили информацию об оплате.
    ...
    $$("[data-category]")
      .forEach((button) => {
        button.addEventListener(
          "click",
          () => {
            // Сохраняем горизонтальную позицию ленты категорий.
            // Иначе после render() список снова прыгает к «Все».
            const categoryStrip =
              button.closest(".catalog-tools");
            const scrollLeft =
  ...
  function saveCheckoutData() {
    ...
    if (nameInput) {
      state.checkout.name = nameInput.value.trim();
    }

    const contactInput = $("#checkoutContact");
    if (contactInput) {
      state.checkout.contact = contactInput.value.trim();
    }

    const commentInput = $("#checkoutComment");
    if (commentInput) {
      state.checkout.comment = commentInput.value.trim();
    }
  }
  ...
  function saveCheckoutReceiving() {
    ...
    const cityInput = $("#deliveryCity");
    if (cityInput) {
      state.checkout.city = cityInput.value.trim();
    }

    const streetInput = $("#deliveryStreet");
    if (streetInput) {
      state.checkout.street = streetInput.value.trim();
    }

    const houseInput = $("#deliveryHouse");
    if (houseInput) {
      state.checkout.house = houseInput.value.trim();
    }

    const apartmentInput = $("#deliveryApartment");
    if (apartmentInput) {
      state.checkout.apartment = apartmentInput.value.trim();
    }
  }
