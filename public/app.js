...
(() => {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      /*
       * ВАЖНО: создание заказа НЕ означает, что пользователь
       * уже внёс залог. Заявка на проверку отправляется
       * только после отдельного нажатия «Я внёс залог»
       * на странице созданного заказа.
       */
      const firstCreated = created[0];
      if (!firstCreated?.id) {
        throw new Error("Созданный заказ не содержит ID");
      }
