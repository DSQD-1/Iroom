...
(() => {
  ...
  async function reservationPaid(
    ...
    try {
      ...
      if (!realOrderId) {
        throw new Error("Не удалось определить ID заказа");
      }
      ...
      if (data?.order) {
        ...
        state.orders = state.orders.map((order) =>
          String(order.id) === String(data.order.id)
            ? data.order
            : order
        );
        state.route = "order";
    ...
    $$("[data-option-key]")
      .forEach((button) => {
        button.addEventListener(
          ...
          () => {
            const scrollY = window.scrollY || window.pageYOffset || 0;
            state.selectedOptions[button.dataset.optionKey] = button.dataset.optionValue;
            renderProduct($("#screen"));
            requestAnimationFrame(() => {
              window.scrollTo({ top: scrollY, behavior: "auto" });
            });
