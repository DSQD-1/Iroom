...
(() => {
  ...
  function haptic(type = "light") {
    try {
      if (typeof tg?.HapticFeedback?.impactOccurred === "function") {
        tg.HapticFeedback.impactOccurred(type === "success" ? "medium" : "light");
      }
    } catch {}
  }
  ...
  function checkoutStepReservation(
    ...
    const instruction =
      ...
      "После оплаты нажмите кнопку «Я внёс залог».";
  ...
  async function placeOrder() {
    ...
    try {
      ...
      for (const item of state.cart) {
        ...
        /*
         * Основной ответ сервера: { order: {...} }.
         * Если старый/промежуточный backend вернул 201 без
         * вложенного order, не теряем только что созданную
         * запись: перечитываем заказы пользователя и берём
         * самый свежий заказ, которого ещё нет в created.
         */
        let createdOrder =
          data?.order ||
          data?.created_order ||
        ...
        if (!createdOrder?.id) {
          try {
            ...
            const knownIds = new Set(
              created.map((order) => String(order.id))
            );

            const freshOrder =
              state.orders.find(
                (order) => !knownIds.has(String(order.id))
              );

            if (freshOrder?.id != null) {
              createdOrder = freshOrder;
            }
          } catch (fallbackError) {
            console.warn(
      ...
      /* Всегда открываем страницу именно созданного заказа. */
      state.route = "order";
      render();
      ...
      <div class="screen">
        ...
        ${
          ["pending_payment", "pending"].includes(
            String(order.reservation_status || "")
          )
  ...
  function bindReservationDelegation() {
    const screen = $("#screen");
    if (!screen || screen.dataset.reservationDelegated === "1") return;

    screen.dataset.reservationDelegated = "1";

    screen.addEventListener("click", (event) => {
      const button = event.target.closest("[data-reservation-paid]");
      if (!button || !screen.contains(button)) return;

      event.preventDefault();
      event.stopPropagation();

      const id = button.getAttribute("data-reservation-paid");
      if (!id) {
  ...
  function bindDynamicEvents() {
    ...
    /* reservation button handled by delegated #screen listener */
