...
(() => {
              ...
              ? `
                <div class="info-box">
                  ...
                  <div style="margin-top:6px">
                    ...
                    проверит перевод.
                    После подтверждения вы
                    получите уведомление от бота.
                  </div>
                </div>
              `
              : order.reservation_status ===
  ...
  async function reservationPaid(
    ...
    try {
      ...
      /*
       * Сервер уже вернул обновлённый
       * заказ со статусом
       * awaiting_confirmation.
       *
       * Оставляем пользователя
       * на странице его заказа.
       */
      if (data?.order) {
        state.currentOrder =
          data.order;
