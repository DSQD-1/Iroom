...
(() => {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      */
      ...
      /*
        После создания заказа сразу открываем
        именно страницу созданного заказа.
        Если backend не прислал id, берём
        последний заказ пользователя из API.
      */
      ...
      if (!createdOrderId && state.orders.length) {
        createdOrder = state.orders[0];
        createdOrderId =
          createdOrder?.id ??
          createdOrder?.order_id ??
          createdOrder?.orderId ??
      ...
      if (!createdOrderId) {
        throw new Error(
          "Сервер создал заказ, но не вернул его ID"
        );
      }
  ...
  async function reservationPaid(
    ...
    try {
      ...
      const botChatUrl =
        data?.bot_chat_url ||
        data?.order?.bot_chat_url ||
        "";

      toast(
        "Статус заказа отправлен в чат с ботом"
      );

      /*
       * После подтверждения оплаты залога
       * переводим пользователя в чат именно
       * с нашим ботом. Ссылка содержит ID заказа,
       * поэтому бот покажет статус конкретного заказа.
       */
      if (botChatUrl) {
        const telegram =
          getTelegram();
      ...
      /*
       * Если сервер не смог сформировать
       * ссылку на бота, оставляем пользователя
       * на странице заказа и показываем статус.
       */
      if (data?.order) {
        state.currentOrder =
          data.order;
