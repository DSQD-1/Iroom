...
(() => {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      */
      ...
      /* После создания заказа пользователь переходит на страницу заказа.
         Залог подтверждается только отдельной кнопкой «Я оплатил(а) залог». */
    ...
    screen.innerHTML = `
      <div class="screen profile-page">
        ...
        <section class="order-card" style="margin-bottom:14px">
          <div class="summary-row">
            <span>Телефон</span>
            <button class="text-link" id="profilePhoneButton" type="button">${escapeHtml(user.phone || "Добавить")}</button>
          </div>
        </section>
  ...
  function bindDynamicEvents() {
        ...
        button.addEventListener(
          ...
          () => {
            ...
            requestAnimationFrame(() => {
              ...
              if (nextStrip) {
                nextStrip.scrollLeft = categoryScrollLeft;
              }
            });
          }
        );
    ...
    $("#profilePhoneButton")?.addEventListener("click", async () => {
      const current = state.me?.phone || "";
      const phone = prompt("Введите номер телефона", current);
      if (phone === null) return;
      const value = phone.trim();
      try {
        const data = await api("/api/me", { method: "PUT", body: { phone: value } });
        state.me = data.user || state.me;
        render();
        toast("Телефон сохранён");
      } catch (error) {
        toast(error.message || "Не удалось сохранить телефон");
      }
    });
  }
