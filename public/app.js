...
(() => {
  ...
  function renderCatalog(screen) {
    /*
      ...
      ДО перерисовки каталога.

      Раньше screen.innerHTML полностью пересоздавал
      .catalog-tools, из-за чего scrollLeft сбрасывался
      в 0 и лента возвращалась к кнопке «Все».
    */
    const oldCategoryBar =
      screen.querySelector(".catalog-tools");

    const savedCategoryScroll =
      oldCategoryBar
        ? oldCategoryBar.scrollLeft
        : 0;
    ...
    /*
      После innerHTML .catalog-tools уже новый элемент.
      Возвращаем ему сохранённую горизонтальную позицию.

      requestAnimationFrame нужен, чтобы браузер сначала
      создал и отрисовал новую ленту, а затем применил
      scrollLeft.
    */
    const newCategoryBar =
  ...
  function renderDrawerCategories() {
    ...
    if (!container.dataset.categoryDelegated) {
      container.dataset.categoryDelegated = "1";

      container.addEventListener("click", (event) => {
        const button = event.target.closest("[data-drawer-category]");
        if (!button || !container.contains(button)) return;

        event.preventDefault();
        event.stopPropagation();

        state.categoryId = button.dataset.drawerCategory || null;
        state.search = "";
        state.route = "catalog";
  ...
  function bindDynamicEvents() {
    ...
    /* Категории обрабатываются через делегирование на #screen.
       Это стабильно работает и для длинного списка категорий,
       который полностью перерисовывается при поиске. */
    const screen = $("#screen");

    if (screen && !screen.dataset.categoryDelegated) {
      screen.dataset.categoryDelegated = "1";
