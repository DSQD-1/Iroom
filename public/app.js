...
(() => {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      /*
       * После создания заказа НЕ отправляем reservation-paid.
       * Залог подтверждается только отдельной кнопкой
       * на странице созданного заказа.
       */

      const firstCreated = created[0];

      if (!firstCreated || firstCreated.id == null) {
        throw new Error("Сервер не вернул ID созданного заказа");
      }
      ...
      .forEach((button) => {
        button.addEventListener(
          ...
          (event) => {
            ...
            const id =
              button.dataset.reservationPaid ||
