...
(() => {
  ...
  function productCard(product) {
    const media = mediaOf(product)
      ...
      .slice(0, 20);
    const fallback = imageOf(product);
    ...
    const slides = media.length
      ...
      : fallback
        ...
        `
        : `
          <div class="product-media-slide product-media-empty">
            IR
          </div>
        `;
      ...
      ? `
        <div class="product-media-dots">
          ${media
            .map(
              (_, index) => `
                <span class="product-media-dot ${index === 0 ? "active" : ""}"></span>
              `
            )
            .join("")}
        </div>
    ...
    return `
      ...
      >
        <div class="product-image product-media-carousel">
  ...
  function checkoutStepReservation(total) {
    const amount = num(
      state.config.reservation_amount
    );

    const rawCard = state.config.reservation_card;
    ...
    const instruction =
      ...
      "Переведите залог по указанным реквизитам, затем нажмите «Я перевёл(а) залог».";
      ...
      <div class="reservation-card">
        ...
        <div class="requisites">
          ...
          <div class="requisites-row">
            ...
            <strong>
              ${escapeHtml(
                cardNumber || "Реквизиты уточняются"
              )}
            </strong>
          </div>
        ...
        <div class="info-box">
          ...
          <br><br>
          После подтверждения залога оставшаяся сумма оплачивается наличными при получении.
        </div>

        <button
          class="primary-button"
          data-action="place-order-and-pay"
          type="button"
  ...
  async function placeOrder(markPaid = false) {
  ...
  const aiState = { open:false, loading:false, history:[] };

  function aiMarkup() {
    const admin = isAdmin();
    return `
      <div class="iroom-ai-overlay" id="iroomAIOverlay" aria-hidden="true">
        <div class="iroom-ai-backdrop" data-ai-close="1"></div>
        <section class="iroom-ai-panel" role="dialog" aria-modal="true" aria-label="IRoom AI">
          <header class="iroom-ai-header">
            <div class="iroom-ai-title-wrap">
              <div class="iroom-ai-avatar">✦</div>
              <div>
                <div class="iroom-ai-title">IRoom AI</div>
                <div class="iroom-ai-subtitle">${admin ? "Режим администратора" : "Помощник магазина"}</div>
              </div>
            </div>
            <button class="iroom-ai-close" type="button" data-ai-close="1" aria-label="Закрыть">×</button>
          </header>
          <div class="iroom-ai-messages" id="iroomAIMessages"></div>
          <div class="iroom-ai-prompts" id="iroomAIPrompts">
            ${admin ? `
              <button type="button" data-ai-prompt="Покажи товары, которые сейчас есть в каталоге">Каталог</button>
              <button type="button" data-ai-prompt="Создай новый товар">Создать товар</button>
              <button type="button" data-ai-prompt="Какие цены сейчас у iPhone?">Цены</button>
            ` : `
              <button type="button" data-ai-prompt="Помоги выбрать iPhone">Выбрать iPhone</button>
              <button type="button" data-ai-prompt="Какие iPhone есть до 150000 ₽?">До 150 000 ₽</button>
              <button type="button" data-ai-prompt="Какие товары есть в магазине?">Каталог</button>
            `}
          </div>
          <form class="iroom-ai-form" id="iroomAIForm">
            <input id="iroomAIInput" type="text" maxlength="4000" autocomplete="off" placeholder="${admin ? "Напишите команду для IRoom AI…" : "Спросите про товары…"}">
            <button id="iroomAISend" type="submit" aria-label="Отправить">➤</button>
          </form>
        </section>
      </div>`;
  }
  ...
  function appendAIMessage(role, text) {
    const box = $("#iroomAIMessages");
    if (!box) return;
    const message = document.createElement("div");
    message.className = `iroom-ai-message ${role === "user" ? "user" : "assistant"}`;
  ...
  function setAILoading(loading) {
    ...
    const button = $("#iroomAISend");
    const input = $("#iroomAIInput");
    if (button) { button.disabled = loading; button.textContent = loading ? "…" : "➤"; }
    if (input) input.disabled = loading;
  }

  async function sendAIMessage(value) {
    ...
    try {
      const data = await api("/api/ai/chat", {
        method:"POST",
        body:JSON.stringify({ message, history:aiState.history.slice(-8) })
      });
      const reply = String(data.reply || "Не удалось получить ответ.");
      appendAIMessage("assistant", reply);
      aiState.history.push({ role:"assistant", content:reply });
      if (data.applied?.applied) {
        try { await loadInitialData(); render(); } catch {}
      }
    } catch (error) {
      appendAIMessage("assistant", `⚠️ ${error.message || "Не удалось связаться с IRoom AI"}`);
    } finally {
      setAILoading(false);
      input?.focus();
    }
  ...
  function openAI() {
    closeDrawer();
    let overlay = $("#iroomAIOverlay");
    if (!overlay) {
      document.body.insertAdjacentHTML("beforeend", aiMarkup());
      overlay = $("#iroomAIOverlay");
      overlay?.addEventListener("click", (event) => {
        const close = event.target.closest("[data-ai-close]");
        if (close) closeAI();
        const prompt = event.target.closest("[data-ai-prompt]");
        if (prompt) sendAIMessage(prompt.dataset.aiPrompt || "");
      });
      $("#iroomAIForm")?.addEventListener("submit", (event) => {
        event.preventDefault();
        sendAIMessage($("#iroomAIInput")?.value || "");
      });
    }
    aiState.open = true;
    overlay.classList.add("open");
    overlay.setAttribute("aria-hidden", "false");
    const messages = $("#iroomAIMessages");
    if (messages && !messages.children.length) {
      appendAIMessage("assistant", isAdmin()
        ? "Привет! Я IRoom AI. Могу посмотреть каталог, изменить цену или создать товар прямо из чата."
        : "Привет! Я IRoom AI. Могу помочь выбрать товар, сравнить модели и найти подходящий вариант в каталоге.");
    }
    requestAnimationFrame(() => $("#iroomAIInput")?.focus());
  }

  function closeAI() {
    const overlay = $("#iroomAIOverlay");
    if (!overlay) return;
    aiState.open = false;
  ...
  function bindDynamicEvents() {
    ...
    $$(".product-media-carousel").forEach((carousel) => {
      const track = carousel.querySelector(".product-media-track");
      const dots = [...carousel.querySelectorAll(".product-media-dot")];
