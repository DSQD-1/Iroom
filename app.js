...
app.post(
  ...
  async (
    ...
    try {
      ...
      if (!order) {
        // Заказ уже записан в БД. Не считаем создание неудачным из-за
        // кратковременной задержки чтения. Собираем минимальный объект
        // из данных INSERT, а дальнейшие операции используют его id.
        const fallbackResult = await db.execute({
          sql: `SELECT * FROM orders WHERE id = ? LIMIT 1`,
          args: [insertedOrderId]
        });
        const fallbackOrder = fallbackResult.rows?.[0] || null;
        if (!fallbackOrder) {
          throw new Error("Заказ создан, но база данных не вернула его данные. Повторите попытку через несколько секунд.");
        }
        order = fallbackOrder;
      }
...
app.put(
  ...
  async (
    ...
    try {
      ...
      if (!order) {
        // Статус уже записан в БД. Не падаем на order.id,
        // если Turso временно не вернул строку повторным SELECT.
        return res.json({
          ok: true,
          order: {
            id,
            display_id: orderDisplayId(id),
            status
          }
        });
      }
      ...
      if (order.telegram_user_id) {
        try {
          await sendCustomerOrderStatus(order, order.telegram_user_id);
        } catch (notifyError) {
