...
app.post(
  ...
  async (
    ...
    try {
      ...
      // Turso/libSQL may not expose lastInsertRowid consistently.
      // Find the order we just created by the authenticated Telegram user.
      const insertedOrderIdResult =
        await db.execute({
          sql: `
            SELECT id
            FROM orders
      ...
      const insertedOrderId =
        integerId(
          insertedOrderIdResult.rows?.[0]?.id
        );

      if (!insertedOrderId) {
        throw new Error(
          "Не удалось определить ID созданного заказа"
        );
      }
