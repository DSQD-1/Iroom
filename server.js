...
if (!TURSO_DATABASE_URL || !TURSO_AUTH_TOKEN) {
  console.error(
    "ERROR: TURSO_DATABASE_URL / TURSO_AUTH_TOKEN are missing"
  );
  process.exit(1);
}
...
function calculatePrice(
  ...
  for (
    const [group, options]
    of Object.entries(priceOptions)
  ) {
    if (!options.length) {
      continue;
    }
    ...
    if (!found) {
      return {
        ok: false,
        error:
          `Недопустимое значение параметра: ${group}`
      };
    }
...
/*
  Старая база users может иметь
  совершенно другую структуру.

  Поэтому здесь определяется,
  какие колонки реально существуют.
*/
async function getUserColumns() {
  return getColumns("users");
}
...
/* =========================================================
   FIXED USER COMPATIBILITY
========================================================= */

/*
  ГЛАВНЫЙ ФИКС.

  Старая база IRoom не обязательно имеет users.id.

  Поэтому ensureUser НИКОГДА:
    - не делает SELECT id FROM users;
    - не делает UPDATE users WHERE id;
    - не использует ON CONFLICT.
...
async function migrate() {
  ...
  /*
    ...
    Никаких ALTER TABLE для users.id.
    Никаких UNIQUE/ON CONFLICT.

    Существующую таблицу users
    не пересоздаём.
  */
