...
// Admin panel: support both public/admin.html and a root-level admin.html.
...
<b>Сумма:</b> ${formatPrice(order.price)}
<b>Залог:</b> ${formatPrice(order.reservation_amount)}

Ниже можно открыть страницу заказа в IRoom.
  `.trim();
        ...
        [
          {
            text: "Открыть заказ",
            web_app: {
              url: `${MINIAPP_URL}/index.html?order=${encodeURIComponent(order.id)}`
            }
          }
        ],
        [
...
async function getOrderById(
  ...
  const numericId = integerId(id);
  if (!numericId) return null;

  // Turso/LibSQL может на очень короткое время отдать пустой SELECT
  // сразу после INSERT. Повторяем чтение несколько раз.
...
app.post(
  ...
  async (req, res) => {
    try {
      const telegramUserId = normalizeTelegramId(req.telegramUser.id);
      const phone = stringValue(req.body?.phone).trim();
      if (!phone) return res.status(400).json({ error: "Введите номер телефона" });
      if (phone.length > 40) return res.status(400).json({ error: "Некорректный номер телефона" });

      await ensureUser(req.telegramUser);
      await db.execute({
        sql: `UPDATE users SET phone = ?, first_name = ?, last_name = ?, username = ?, updated_at = CURRENT_TIMESTAMP WHERE telegram_user_id = ?`,
        args: [phone, stringValue(req.telegramUser.first_name), stringValue(req.telegramUser.last_name), stringValue(req.telegramUser.username), telegramUserId]
      });

      const result = await db.execute({
        sql: `SELECT * FROM users WHERE telegram_user_id = ? LIMIT 1`,
        args: [telegramUserId]
      });

      res.json({ user: result.rows[0] || { telegram_user_id: telegramUserId, phone } });
    } catch (error) {
      console.error("POST /api/me/phone:", error);
      res.status(500).json({ error: "Не удалось сохранить номер телефона" });
    }
  }
);
    ...
    try {
      ...
      await db.execute({
        sql: `
          ...
          WHERE telegram_user_id = ?
        `,
        args: [phone, telegramUserId]
      });

      // Если пользователь ещё не был создан, создаём его с актуальными данными Telegram.
      ...
      if (!result.rows.length) {
        await ensureUser(req.telegramUser);
        await db.execute({
          sql: `UPDATE users SET phone = ?, updated_at = CURRENT_TIMESTAMP WHERE telegram_user_id = ?`,
          args: [phone, telegramUserId]
        });
      ...
      const orders =
        result.rows.map(
          order => {
            const resolvedId =
              integerId(order.id) ||
              integerId(order.order_id) ||
              integerId(order.rowid);
...
app.post(
  ...
  async (
    ...
    try {
      ...
      let insertedOrderId =
        integerId(result.rows?.[0]?.id) ||
        integerId(result.lastInsertRowid);

      if (!insertedOrderId) {
        const latest = await db.execute({
          sql: `SELECT id FROM orders WHERE telegram_user_id = ? ORDER BY id DESC LIMIT 1`,
          args: [normalizeTelegramId(req.telegramUser.id)]
        });
        insertedOrderId = integerId(latest.rows?.[0]?.id);
      }
      ...
      // Не блокируем оформление из-за кратковременной задержки SELECT после INSERT.
      // Сам INSERT уже успешно записан, а id получен через RETURNING/lastInsertRowid.
      let order = await getOrderById(insertedOrderId);

      if (!order) {
        const row = result.rows?.[0] || {};
        order = {
          id: insertedOrderId,
          telegram_user_id: normalizeTelegramId(req.telegramUser.id),
          user_id: user?.id || null,
          product_id: productId,
          variant_id: variantId,
          product_name: stringValue(req.body.product_name) || product.name,
          ...
          reservation_card_id: reservationCard?.id || null,
          fulfillment_type: fulfillmentType,
          receiving_type: fulfillmentType,
          ...
          user_first_name: req.telegramUser.first_name || "",
          user_last_name: req.telegramUser.last_name || "",
          user_username: req.telegramUser.username || "",
      ...
      const responseOrder = {
        ...order,
        id: insertedOrderId,
        display_id: orderDisplayId(insertedOrderId),
...
app.post(
  ...
  async (
    ...
    try {
      ...
      // Turso/libSQL иногда может дать отложенное чтение сразу после INSERT.
      // Если заказ по ID ещё не читается, ищем последний заказ этого Telegram-пользователя
      // с ожидающим залогом. Это тот же заказ, который только что был создан.
      if (!order) {
        const ownerId = normalizeTelegramId(req.telegramUser.id);
          ...
          const latest = await db.execute({
            sql: `
              ...
              WHERE o.telegram_user_id = ?
                AND o.reservation_status IN ('pending_payment','pending')
              ORDER BY o.id DESC LIMIT 1
            `,
            args: [ownerId]
          });
          order = latest.rows?.[0] || null;
          if (!order) await new Promise(resolve => setTimeout(resolve, 120));
        }
      }
      ...
      const updated = updateResult.rows?.[0] || await getOrderById(Number(order.id));
      const responseSource = updated || order;
      const responseOrder = {
        ...responseSource,
        ...
        selected_options:
          safeJsonParse(
            responseSource.selected_options,
            {}
          )
      };
...
app.put(
  ...
  async (
    ...
    try {
      ...
      let order = updateResult.rows?.[0] || null;
      if (!order) order = await getOrderById(id);
      ...
      const responseOrder = {
        ...order,
        id: integerId(order.id) || id,
        status,
        display_id: orderDisplayId(integerId(order.id) || id),
        selected_options: safeJsonParse(order.selected_options, {})
      };

      if (responseOrder.telegram_user_id) {
        try {
          await sendCustomerOrderStatus(responseOrder, responseOrder.telegram_user_id);
        } catch (notifyError) {
          console.error("ORDER STATUS NOTIFY:", notifyError);
        }
      }
        ...
        sql: `
          ...
          SET
            ...
            reservation_paid_at = CASE
              WHEN ? = 'confirmed'
              THEN COALESCE(reservation_paid_at, CURRENT_TIMESTAMP)
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        if (startMatch) {
          const orderId = integerId(startMatch[1]);
