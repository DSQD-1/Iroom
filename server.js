...
async function insertImportedProduct(
  product,
  categoryId
) {
  const numericPrice =
...
async function importPriceListOnce() {
  ...
  try {
    ...
    console.error(
      "❌ Не удалось загрузить price-list.js:",
      error.message
    );
    ...
    return {
      ...
      skipped: true,
      error:
        "Файл price-list.js не найден или содержит ошибку"
    };
  }
  ...
  if (
    ...
    console.error(
      "❌ price-list.js должен экспортировать массив"
    );
    ...
    return {
      ...
      skipped: true,
      error:
        "price-list.js должен содержать массив"
    };
  }
  ...
  if (
    ...
    console.log(
      "PRICE LIST: импорт уже выполнялся — пропускаем"
    );
  ...
  console.log(
    "       IROOM PRICE LIST IMPORT"
  );
  console.log(
    "======================================"
  );
  console.log(
    `Позиций в price-list.js: ${priceList.length}`
  );
  console.log("");
  ...
  for (
    ...
    try {
      ...
      if (!category) {
        console.error(
          `❌ Категория не найдена: ${product.category}`
        );
