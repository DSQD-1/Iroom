...
/* =========================================================
   MULTER
   Файл держим в памяти и сразу сохраняем в Turso.
   ========================================================= */
...
async function isAdminTelegramId(telegramId) {
  ...
  if (ADMIN_ID && String(telegramId) === String(ADMIN_ID)) {
    return true;
  }
...
async function requireAdmin(req, res, next) {
  try {
    ...
    const admin = await isAdminTelegramId(telegramUser.id);
...
app.delete("/api/admin/categories/:id", requireAdmin, async (req, res) => {
  try {
    ...
    if (Number(products.rows[0].count) > 0) {
      return res.status(400).json({
        ok: false,
        error: "Сначала перенесите товары из этой категории"
      });
    }
