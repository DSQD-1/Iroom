...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ user: updated.rows[0] || { ...user, phone } });
    } catch (error) {
...
app.post(
  ...
  async (
    ...
    try {
      ...
      // Turso/libSQL может вернуть INSERT ... RETURNING как строку,
      // а чтение сразу после INSERT иногда бывает недоступно в том же запросе.
      // Поэтому сначала используем саму возвращённую строку, а затем делаем
      // несколько безопасных вариантов получения заказа.
      let insertedOrder = result.rows?.[0] || null;
      let insertedOrderId =
        integerId(insertedOrder?.id) ||
      ...
      let order =
        insertedOrder ||
        await getOrderById(insertedOrderId);

      // Если INSERT вернул строку, но без части полей — дополняем её
      // обычным SELECT. Если SELECT ещё не видит запись, сама RETURNING-строка
      // всё равно является созданным заказом и используется дальше.
      if (!order) {
        for (let attempt = 0; attempt < 3 && !order; attempt += 1) {
          await new Promise(resolve => setTimeout(resolve, 80 * (attempt + 1)));
          order = await getOrderById(insertedOrderId);
        }
      }
