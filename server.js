...
async function getVariantsForProduct(productId, onlyActive = false) {
  const result = await db.execute({
    sql: `
      SELECT
        id,
...
app.get("/api/products/:id", async (req, res) => {
  try {
    ...
    const variants =
      await getVariantsForProduct(row.id, true);

    return json(res, 200, {
      ok: true,
      product: mapProduct(row, variants)
    });
  } catch (error) {
    ...
    try {
      ...
      const message = [
        ...
        configuration
          ...
          : "",
        price
          ? `Цена: ${formatPrice(price, currency || "RUB")}`
          : "",
        "",
        `Telegram ID: ${user.id}`,
        user.username
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      for (const row of result.rows) {
        const variants =
          await getVariantsForProduct(row.id);
...
function normalizeVariants(input) {
  ...
  return input
    .map((variant) => ({
      memory: normalize(variant.memory),
      color: normalize(variant.color),
      country: normalize(variant.country),
      sim_type: normalize(
        variant.sim_type
      ),
      price: num(variant.price),
      currency:
        normalize(variant.currency) || "RUB",
      stock: Math.max(
        0,
        Math.floor(num(variant.stock))
      ),
      active:
...
async function saveProduct({
  ...
  await db.execute({
    ...
    args: [
      id,
      name,
      normalize(body.category_id),
      normalize(body.description),
      normalize(body.image_id),
      normalize(body.image_url),
      body.is_new ? 1 : 0,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const parsed =
        await parsePriceListWithAI(text);
...
async function executeAdminAIAction(action) {
  ...
  switch (action.type) {
    case "update_price": {
      const productId =
        normalize(action.product_id);

      const variantId =
        normalize(action.variant_id);
