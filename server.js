...
async function isAdminTelegramId(telegramId) {
  ...
  if (
    ADMIN_ID &&
    String(telegramId) === String(ADMIN_ID)
  ) {
    return true;
  }
...
async function initDatabase() {
  ...
  /* =======================================================
     ...
     ВАЖНО:
     Настройки создаются только если их ещё нет.

     Поэтому сохранённый через админку
     contact_username НЕ перезаписывается
     при каждом запуске Render.
     ======================================================= */
  ...
  async (req, res) => {
    try {
      ...
      */
      ...
      for (
        const row of saved.rows
      ) {
        savedSettings[row.key] =
          row.value;
      }
