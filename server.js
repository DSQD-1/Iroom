...
<b>Телефон:</b> ${escapeTelegramHtml(
...
<b>Время заявки:</b> ${escapeTelegramHtml(
  ...
  const replyMarkup = {
    inline_keyboard: [
      [
        { text: "Подтвердить залог", callback_data: `reservation_confirm:${order.id}` },
        { text: "Отклонить", callback_data: `reservation_reject:${order.id}` }
      ],
      [
        {
          text: "Открыть заказ",
          web_app: { url: `${MINIAPP_URL}/admin.html?order=${order.id}` }
        }
      ]
    ]
  };
...
app.get(
  ...
  async (
    ...
    try {
      ...
      const customerContact = stringValue(req.body.contact || "");
      if (customerContact) {
        await db.execute({
          sql: `UPDATE users SET phone = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
          args: [customerContact, user.id]
        });
      }
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ user: updated.rows[0] || user });
    } catch (error) {
      console.error("UPDATE ME:", error);
      res.status(500).json({ error: "Не удалось сохранить телефон" });
    }
  }
);
    ...
    try {
      ...
      if (promoCode) {
        ...
        if (Number(used.rowsAffected || 0) !== 1) {
          console.error("PROMO USE COUNT UPDATE FAILED", promoCode, insertedOrderId);
        }
      }
  ...
  async (
    ...
    try {
      ...
      try {
        ...
        console.error("Customer status notification:", notificationError.message);
      }
        ...
        if (data.startsWith("reservation_confirm:") || data.startsWith("reservation_reject:")) {
          ...
          if (orderId) {
            ...
            if (order && isAdminId(adminId)) {
              ...
              await notifyCustomerReservation({ ...updated, display_id: orderDisplayId(orderId) });
              await answerCallbackQuery(callback.id, nextStatus === "confirmed" ? "Залог подтверждён" : "Залог отклонён");
            } else {
