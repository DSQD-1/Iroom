...
async function getReservationAmount(card = null) {
  const cardAmount = Math.max(0, numberValue(card?.amount));
...
async function migrate() {
  ...
  // Existing Turso databases may have older versions of these tables.
  // CREATE TABLE IF NOT EXISTS does not add columns to an existing table,
  // so every column used by the API is explicitly migrated here.
...
app.post(
  ...
  async (
    ...
    try {
      ...
      if (reservationAmount > 0 && !reservationCard) {
        return res.status(409).json({
          error: "Для заказа включён залог, но реквизиты залога не настроены. Администратору нужно добавить активную карту/реквизиты в панели."
        });
      }
    ...
    try {
      ...
      if (currentStatus === "awaiting_confirmation") {
        return res.status(400).json({
          error: "Заявка уже ожидает проверки"
        });
      }

      if (currentStatus === "confirmed") {
        return res.status(400).json({
          error: "Залог уже подтверждён"
        });
      }

      if (!["pending_payment", "pending", "rejected"].includes(currentStatus)) {
        return res.status(400).json({
          error: "Сейчас нельзя отправить подтверждение оплаты залога"
        });
      }
        ...
        sql: `
          ...
          SET
            ...
            reservation_paid_at = CASE
              WHEN ? = 'awaiting_confirmation' THEN CURRENT_TIMESTAMP
...
function compactProductForAI(product, categoryMap = {}) {
  const options = parsePriceOptions(product.price_options);

  return {
    id: Number(product.id),
    category_id: product.category_id ? Number(product.category_id) : null,
    category: categoryMap[String(product.category_id)] || "",
    name: String(product.name || ""),
    description: String(product.description || ""),
    price: numberValue(product.price),
    old_price: numberValue(product.old_price),
    is_new: Number(product.is_new) === 1,
    options
  };
}
...
async function getAICatalog() {
  const [productsResult, categoriesResult] = await Promise.all([
    db.execute(`SELECT * FROM products ORDER BY id DESC LIMIT 150`),
    db.execute(`SELECT id, name FROM categories ORDER BY id ASC`)
  ]);

  const categoryMap = {};
  for (const row of categoriesResult.rows) {
    categoryMap[String(row.id)] = String(row.name || "");
  }

  return {
    products: productsResult.rows.map((product) =>
      compactProductForAI(product, categoryMap)
    ),
...
function parseAIJson(text) {
  ...
  return safeJsonParse(fenced, null);
}

async function callOpenAIForIRoom({ message, history, admin, catalog }) {
  if (!OPENAI_API_KEY) {
    throw new Error(
      "OPENAI_API_KEY не настроен. Добавьте его в Environment Variables Render."
    );
  }
  ...
  const historyText = Array.isArray(history)
    ? history
        .slice(-8)
        .map((item) => `${item?.role === "assistant" ? "AI" : "Клиент"}: ${String(item?.content || "")}`)
        .join("\n")
    : "";

  const instructions = admin
    ? `Ты — IRoom AI, помощник интернет-магазина IRoom и ассистент администратора.

Ты видишь реальный каталог из базы данных. Не придумывай товары, цены или ID.

У администратора есть право сразу изменять каталог. Если запрос явно просит создать или изменить товар, сформируй action. После успешного действия сервер сам сообщит результат.

Разрешённые действия:
- create_product — создать товар;
- update_product — изменить существующий товар;
- set_price — изменить только цену товара;
- none — просто ответить без изменения.

Никогда не используй действие для обычного вопроса.
Если пользователь не указал достаточно данных для изменения, action=none и попроси уточнение.
Для update_product и set_price обязательно используй существующий product_id из каталога.
Для create_product name обязателен. Если цена не указана, не выдумывай её — попроси цену.
Категорию выбирай только из существующих категорий по id. Если подходящей категории нет, оставь category_id=null.

Верни строго JSON по заданной схеме. reply — короткий ответ на русском языке.`
    : `Ты — IRoom AI, консультант магазина IRoom.

Помогай покупателю выбрать товар, сравнивать реальные товары, узнавать цены, варианты памяти, цвета, SIM и регионы.
Используй только данные реального каталога ниже. Не придумывай наличие или цену.

Ты НЕ являешься администратором и никогда не можешь изменять товары, цены, категории или настройки.
Если пользователь просит что-либо изменить в магазине, вежливо скажи, что это может сделать только администратор.

Верни строго JSON по заданной схеме. reply — естественный короткий ответ на русском языке.`;

  const userPrompt = [
    `Текущий каталог IRoom:\n${catalogText}`,
  ...
  const schema = {
    ...
    properties: {
      ...
      action: {
        type: "string",
        enum: ["none", "create_product", "update_product", "set_price"]
      },
      ...
      price_options: { type: ["string", "null"] }
    },
    required: [
      "reply",
      "action",
  ...
  const parsed = parseAIJson(extractAIText(data));
  if (!parsed) {
...
function validateAIPrice(value) {
...
async function executeAIAdminAction(action) {
  const type = stringValue(action?.action || "none");
  ...
  if (type === "set_price") {
    const productId = integerId(action.product_id);
    if (!productId) throw new Error("ИИ не указал существующий товар");

    const price = validateAIPrice(action.price);
    ...
    if (!existing.rows.length) {
      throw new Error("Товар для изменения цены не найден");
    }
  ...
  if (type === "update_product") {
    ...
    const nextName = action.name !== null ? stringValue(action.name) : stringValue(current.name);
    if (!nextName) throw new Error("Название товара не может быть пустым");

    const nextPrice = action.price !== null
      ? validateAIPrice(action.price)
      : numberValue(current.price);

    const nextOldPrice = action.old_price !== null
      ? Math.max(0, numberValue(action.old_price))
      : numberValue(current.old_price);

    let categoryId = current.category_id ? integerId(current.category_id) : null;
    if (action.category_id !== null) {
      categoryId = integerId(action.category_id);
      if (categoryId) {
        const category = await db.execute({
          sql: `SELECT id FROM categories WHERE id = ? LIMIT 1`,
          args: [categoryId]
        });
        if (!category.rows.length) throw new Error("Указанная категория не существует");
      }
    }
    ...
    const imageUrl = action.image_url !== null
      ? stringValue(action.image_url)
      : stringValue(current.image_url);

    const isNew = action.is_new !== null
      ? (action.is_new ? 1 : 0)
      : Number(current.is_new) === 1 ? 1 : 0;

    const priceOptions = action.price_options !== null
      ? (safeJsonParse(action.price_options, {}) || {})
      : safeJsonParse(current.price_options, {});

    await db.execute({
      sql: `
        UPDATE products
        SET category_id = ?, name = ?, description = ?, price = ?, old_price = ?, image_url = ?, price_options = ?, is_new = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `,
      args: [
        categoryId,
  ...
  if (type === "create_product") {
    ...
    if (!name) throw new Error("Для создания товара нужно название");
    if (action.price === null || action.price === undefined) {
      throw new Error("Для создания товара нужно указать цену");
    }

    const price = validateAIPrice(action.price);
    const categoryId = integerId(action.category_id);
    ...
    const result = await db.execute({
      ...
      args: [
        ...
        stringValue(action.image_url),
        "[]",
        JSON.stringify(safeJsonParse(action.price_options, {}) || {}),
        action.is_new ? 1 : 0
      ]
    });
  ...
  throw new Error("Неизвестное действие ИИ");
}

const aiRateLimit = new Map();

function checkAIRateLimit(telegramId, admin) {
  const now = Date.now();
  const key = String(telegramId || "");
  const current = aiRateLimit.get(key) || { start: now, count: 0 };
  ...
  const limit = admin ? 60 : 20;
  if (current.count >= limit) {
    aiRateLimit.set(key, current);
    return false;
  }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const admin = isAdminId(req.telegramUser?.id);

      if (!checkAIRateLimit(req.telegramUser?.id, admin)) {
      ...
      const aiResult = await callOpenAIForIRoom({
        message,
        history,
        admin,
        catalog
      ...
      let applied = null;
      let reply = stringValue(aiResult.reply) || "Готово.";

      if (admin && aiResult.action && aiResult.action !== "none") {
        try {
          applied = await executeAIAdminAction(aiResult);

          if (applied?.applied) {
            const productName = stringValue(applied.product?.name) || "товар";
            const productId = integerId(applied.product?.id);
            const price = applied.product?.price !== undefined
              ? formatPrice(applied.product.price)
              : "";

            if (applied.action === "create_product") {
              reply = `✅ Создал товар «${productName}»${price ? ` — ${price}` : ""}. ID: ${productId}.`;
            } else if (applied.action === "set_price") {
              reply = `✅ Цена товара «${productName}» изменена на ${price}.`;
            } else {
              reply = `✅ Товар «${productName}» обновлён${price ? ` — ${price}` : ""}.`;
            }
          }
        } catch (actionError) {
          console.error("IRoom AI action:", actionError);
          return res.status(400).json({
            reply: `⚠️ Не удалось применить изменение: ${actionError.message}`,
            admin,
      ...
      console.error("IRoom AI:", error);
      res.status(500).json({
        error: error.message || "Не удалось связаться с IRoom AI"
      });
    }
  }
);
      ...
      if (text === "/start" || text === "/app") {
        await sendTelegramMessage(
          ...
          [
            ...
            "",
            "📱 Здесь можно посмотреть товары и оформить заказ.",
            "",
...
async function setupWebhook() {
  try {
    const url = `${MINIAPP_URL}/telegram/webhook`;
