...
function orderStatusLabel(status) {
  ...
  return labels[String(status)] || status || "Новый";
}
...
function hasPriceOptions(priceOptions) {
  return PRICE_OPTION_GROUPS.some(
    (group) =>
      Array.isArray(priceOptions[group]) &&
      priceOptions[group].length > 0
  );
}
...
function calculatePriceWithOptions(
  ...
  for (const group of PRICE_OPTION_GROUPS) {
    ...
    if (!found) {
      throw new Error(
        `Недопустимое значение параметра: ${selectedName}`
      );
    }
      ...
      [
        {
          ...
          callback_data:
            `order_status:${orderId}:confirmed`
        }
      ],
      [
        {
      ...
      [
        {
          ...
          callback_data:
            `reservation:${orderId}:confirmed`
        },
        {
          text: "Отклонить бронь",
          callback_data:
            `reservation:${orderId}:rejected`
        }
      ]
    ]
  };
...
async function notifyCustomerReservation(
  ...
  if (
    ...
    text =
      ...
      `Заказ: ${escapeTelegramHtml(
        orderDisplayId(order.id)
      )}\n` +

      `Товар: ${escapeTelegramHtml(
        order.product_name
      )}\n` +
        ...
        await db.execute({
          sql: `
            SELECT
              ...
              pp.address AS pickup_point_address,
              pp.working_hours
                AS pickup_point_working_hours,
              pp.map_url
                AS pickup_point_map_url,
              pp.video_url
                AS pickup_point_video_url
            FROM orders o
            LEFT JOIN pickup_points pp
              ON pp.id =
                o.pickup_point_id
        ...
        await db.execute({
          sql: `
            SELECT
              o.*,
              pp.name
                AS pickup_point_name,
              pp.address
                AS pickup_point_address
            FROM orders o
            LEFT JOIN pickup_points pp
              ON pp.id =
                o.pickup_point_id
      ...
      const result =
        await db.execute(`
          SELECT
            ...
            rc.card_number
              AS reservation_card_number,

            rc.recipient_name
              AS reservation_recipient_name,

            rc.bank_name
              AS reservation_bank_name
