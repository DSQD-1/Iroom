...
function customerName(user) {
  ...
  return full || stringValue(user.username, "Покупатель");
}
...
function normalizeOptionItem(item) {
  ...
  let surcharge = numberValue(item.surcharge, 0);
...
function normalizeOptionGroup(value) {
  ...
  for (const item of value) {
    ...
    const exists = result.some(
      (current) =>
        current.name.toLowerCase() === normalized.name.toLowerCase()
    );
...
function normalizePriceOptions(value) {
  ...
  return {
    colors: normalizeOptionGroup(source.colors),
    memories: normalizeOptionGroup(source.memories),
    sims: normalizeOptionGroup(source.sims),
    regions: normalizeOptionGroup(source.regions)
  };
}
...
function hasPriceOptions(options) {
  return PRICE_OPTION_GROUPS.some(
    (group) => Array.isArray(options[group]) && options[group].length > 0
  );
}

function getSelectedValue(source, keys) {
...
function findOption(options, selectedValue) {
  if (!selectedValue) {
    return null;
  }
...
function calculatePriceWithOptions(product, selectedInput) {
  const basePrice = Math.round(numberValue(product.price, 0));
  ...
  const input =
    selectedInput && typeof selectedInput === "object"
      ? selectedInput
      : {};
  ...
  const groups = [
    ...
    {
      group: "sims",
      keys: ["sim", "sim_type", "simType", "sims"],
      outputKey: "sim"
    },
    {
      group: "regions",
  ...
  for (const definition of groups) {
    const available = options[definition.group];
    ...
    if (!selectedValue) {
      return {
        ok: false,
        error: `Выберите: ${definition.group}`
      };
    }

    const selected = findOption(available, selectedValue);

    if (!selected) {
      return {
        ok: false,
        error: `Недопустимое значение для ${definition.group}`
      };
    }

    selectedOptions[definition.outputKey] = selected.name;

    total += selected.surcharge;

    variantParts.push(selected.name);
  }
...
function buildVariantName({
  ...
  return [color, memory, sim_type, region]
...
async function ensureProductPriceOptionsColumn() {
  await ensureColumn(
    "products",
    "price_options",
    "TEXT DEFAULT '{}'"
...
async function ensureOrderSelectedOptionsColumn() {
  await ensureColumn(
    "orders",
    "selected_options",
    "TEXT DEFAULT '{}'"
...
async function runMigrations() {
  ...
  await ensureProductPriceOptionsColumn();
  await ensureOrderSelectedOptionsColumn();
}
...
async function telegram(method, body = {}) {
  if (!BOT_TOKEN) {
    return {
      ok: false,
      description: "BOT_TOKEN не задан"
    };
  }
...
function buildOrderTelegramText(order) {
  const selectedOptions = safeJsonParse(
    order.selected_options,
    {}
  );

  const characteristics = Object.entries(selectedOptions)
    .filter(([, value]) => value)
...
async function sendAdminOrderNotification(order) {
  ...
  for (const adminId of ADMIN_IDS) {
    await telegram("sendMessage", {
      ...
      text,
      parse_mode: "HTML",
      reply_markup: buildOrderKeyboard(
        order.id,
        order.status
      )
    });
...
app.get("/api/products", async (_req, res) => {
  try {
    ...
    const products = productsResult.rows.map((product) => ({
      ...
      price_options: priceOptionsForResponse(
        product.price_options
      ),
      variants:
        variantsByProduct[Number(product.id)] || []
    }));
  ...
  try {
    ...
    res.json({
      ...
      product: {
        ...
        variants: variantsResult.rows.map(normalizeVariant)
      }
    });
  } catch (error) {
...
app.get("/api/me", async (req, res) => {
  ...
  res.json({
    ...
    user: user || null,
    is_admin: Boolean(
      user && ADMIN_IDS.includes(String(user.id))
    )
  });
});
...
app.get("/api/settings", async (_req, res) => {
  try {
    ...
    try {
      ...
      // Если таблицы settings пока нет — возвращаем базовые настройки.
    }
...
app.get("/api/orders/:id", async (req, res) => {
  try {
    ...
    order.price = numberValue(order.price);
    order.selected_options = safeJsonParse(
      order.selected_options,
      {}
    );
...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    const priceOptions = normalizePriceOptions(
      product.price_options
    );

    /*
      Новый вариант:
      selected_options / options / price_options_selection
    ...
    */
    ...
    let finalPrice = numberValue(product.price);
    let selectedOptions = {};
    let variantText = "";
    let variantId = null;

    if (hasPriceOptions(priceOptions)) {
      ...
      variantText = calculated.variantText;
    } else {
      /*
        Старые товары с product_variants продолжают работать.
      */

      const requestedVariantId = integerId(
        req.body.variant_id
      );
      ...
      if (requestedVariantId) {
        const variantResult = await db.execute({
          sql: `
            ...
            LIMIT 1
          `,
          args: [requestedVariantId, productId]
        });
        ...
        if (Number(variant.stock) <= 0) {
          return res.status(400).json({
            ok: false,
            error: "Выбранного варианта сейчас нет в наличии"
          });
        }

        variantId = variant.id;
        finalPrice = numberValue(variant.price);
    ...
    /*
      ...
      Сохраняем их в variant_text дополнительно только если
      отдельные поля существуют в текущей БД не предусмотрены.
    */

    const customerNameValue = customerName(user);
  ...
  async (req, res) => {
    try {
      ...
      const price = Math.max(
        0,
        Math.round(numberValue(req.body.price, 0))
      );
      ...
      const categoryId =
        integerId(req.body.category_id) || null;
    ...
    try {
      ...
      const imageUrl =
        ...
        req.body.image_url !== undefined
          ...
          : stringValue(existing.image_url);

      const description =
        req.body.description !== undefined
          ? stringValue(req.body.description)
          : stringValue(existing.description);

      const categoryId =
        req.body.category_id !== undefined
          ? integerId(req.body.category_id)
          : integerId(existing.category_id);
      ...
      res.json({
        ...
        product: {
          ...
          variants: variantsResult.rows.map(
            normalizeVariant
          )
        }
      });
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ok: true,
        variants: result.rows.map(
          normalizeVariant
        )
      });
    } catch (error) {
      console.error("GET variants:", error);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const color = stringValue(req.body.color);
      const memory = stringValue(req.body.memory);
      const simType = stringValue(
        req.body.sim_type ||
          req.body.sim
      );
      ...
      const stock = Math.max(
        0,
        Math.round(numberValue(req.body.stock, 0))
      );
      ...
      console.error("POST variant:", error);
    ...
    try {
      ...
      const color =
        req.body.color !== undefined
          ? stringValue(req.body.color)
          : stringValue(existing.color);

      const memory =
        req.body.memory !== undefined
          ? stringValue(req.body.memory)
          : stringValue(existing.memory);

      const simType =
        req.body.sim_type !== undefined ||
        req.body.sim !== undefined
          ? stringValue(
              req.body.sim_type ||
                req.body.sim
            )
          : stringValue(existing.sim_type);

      const region =
        req.body.region !== undefined
          ? stringValue(req.body.region)
          : stringValue(existing.region);
      ...
      console.error("PUT variant:", error);
