...
async function requireAdmin(req, res, next) {
  try {
    ...
    const admin = await isAdminTelegramId(
      telegramUser.id
    );
  ...
  if (Number(categoryCount.rows[0].count) === 0) {
    await db.batch(
      [
        ...
        ["AirPods", "airpods", 5]
      ].map(([name, slug, sort_order]) => ({
        sql: `
          INSERT INTO categories
          (name, slug, sort_order, active)
          VALUES (?, ?, ?, 1)
        `,
        args: [name, slug, sort_order]
      })),
      "write"
    );
  }
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      /*
        Если фото заменили или удалили —
        старое BLOB-изображение удаляем из Turso.
      */
...
async function setupTelegramBot() {
  ...
  startTelegramPolling(MINIAPP_URL);
}
