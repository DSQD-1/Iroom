...
function normalizePriceOptions(value) {
  const result = emptyPriceOptions();
  ...
  for (const group of PRICE_OPTION_GROUPS) {
    const rawItems =
      Array.isArray(source[group])
        ? source[group]
        : [];

    result[group] =
      rawItems
        .map(normalizePriceOptionItem)
        .filter(Boolean);
  }
...
function parseStoredPriceOptions(value) {
  return normalizePriceOptions(value);
}

function serializePriceOptions(value) {
  return JSON.stringify(
    normalizePriceOptions(value)
  );
}

function findPriceOption(
  options,
  group,
  selectedName
) {
...
function calculateProductPrice(
  ...
  for (const [group, selectedKey] of groups) {
    const selectedName =
    ...
    if (!option) {
      return {
        ok: false,
        error:
          `Выбранная опция "${selectedName}" недоступна`
      };
    }
    ...
    const requiredColumns = [
      {
        name: "color",
        sql:
          "ALTER TABLE product_variants ADD COLUMN color TEXT DEFAULT ''"
      },
      {
        name: "memory",
        sql:
          "ALTER TABLE product_variants ADD COLUMN memory TEXT DEFAULT ''"
      },
      {
        name: "sim_type",
        sql:
          "ALTER TABLE product_variants ADD COLUMN sim_type TEXT DEFAULT ''"
      },
      {
        name: "region",
        sql:
          "ALTER TABLE product_variants ADD COLUMN region TEXT DEFAULT ''"
      }
    ];
...
async function ensureProductPriceOptionsColumn() {
  try {
    ...
    if (
      ...
      console.log(
        "➕ Добавляем products.price_options"
      );

      await db.execute(`
        ALTER TABLE products
        ADD COLUMN price_options TEXT DEFAULT '{}'
      `);
    }

    console.log(
      "✅ Поле products.price_options проверено"
    );
  } catch (error) {
    console.error(
      "❌ Ошибка миграции products.price_options:",
      error
    );
...
app.post(
  ...
  async (
    ...
    try {
      ...
      /*
       * Новая система:
       * сервер считает цену самостоятельно.
       */
      ...
      /*
       * Старая система вариантов.
       * Оставлена для существующих товаров.
       */
      ...
      if (variantId !== null) {
        ...
        /*
         * Старые варианты хранят собственную
         * итоговую цену.
         */
        finalPrice =
          numberValue(
        ...
        /*
         * Для старых товаров без вариантов
         * используем цену самого товара.
         * Цена клиента не является источником
         * истины.
         */
        finalPrice =
          numberValue(
...
app.put(
  ...
  async (
    ...
    try {
      ...
      /*
       * ВАЖНО:
       * если price_options пришёл,
       * полностью заменяем старые опции.
       * Если не пришёл — сохраняем текущие.
       */
      let priceOptions =
        parseStoredPriceOptions(
          current.price_options
        );
...
async function start() {
  try {
     ...
     *
     * Поэтому вручную менять Turso не нужно.
     */
