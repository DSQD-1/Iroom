      ...
      [
        {
          ...
          callback_data: `order_reservation_confirm:${order.id}`
        },
        {
          text: "❌ Отклонить",
          callback_data: `order_reservation_reject:${order.id}`
        }
      ],
      [
        {
...
Залог подтверждён менеджером. Заказ принят в работу.
...
app.post(
  ...
  async (
    ...
    try {
      ...
      // Покупателю сразу приходит уведомление о создании заказа.
      try {
        const telegramId = normalizeTelegramId(responseOrder.telegram_user_id);
        if (telegramId) {
          await sendCustomerOrderStatus(responseOrder, telegramId);
        }
      } catch (notificationError) {
        console.error(
          "Customer new order notification:",
...
app.put(
  ...
  async (
    ...
    try {
      ...
      // Сообщаем покупателю о каждом изменении статуса заказа.
      try {
        const telegramId = normalizeTelegramId(responseOrder.telegram_user_id);
        if (telegramId) {
          await sendCustomerOrderStatus(responseOrder, telegramId);
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        const callbackAdminId = normalizeTelegramId(callback.from?.id);

        if (data.startsWith("order_reservation_confirm:") || data.startsWith("order_reservation_reject:")) {
          if (!isAdminId(callbackAdminId)) {
            await answerCallbackQuery(callback.id, "Нет доступа", true);
            return;
          }

          const isConfirm = data.startsWith("order_reservation_confirm:");
          const id = integerId(data.split(":")[1]);
          const existing = id ? await getOrderById(id) : null;

          if (!existing) {
            await answerCallbackQuery(callback.id, "Заказ не найден", true);
            return;
          }

          if (existing.reservation_status !== "awaiting_confirmation") {
            await answerCallbackQuery(callback.id, "Заявка уже обработана", true);
            return;
          }

          const nextReservationStatus = isConfirm ? "confirmed" : "rejected";
          const nextOrderStatus = isConfirm ? "confirmed" : existing.status;
          ...
          await db.execute({
            sql: `
              ...
              SET
                reservation_status = ?,
                status = ?,
                reservation_paid_at = COALESCE(reservation_paid_at, CURRENT_TIMESTAMP),
                updated_at = CURRENT_TIMESTAMP
              WHERE id = ?
            `,
            args: [nextReservationStatus, nextOrderStatus, id]
          });

          const updated = await getOrderById(id);
          const responseOrder = {
            ...updated,
            display_id: orderDisplayId(updated.id),
            selected_options: safeJsonParse(updated.selected_options, {})
          };

          await notifyCustomerReservation(responseOrder);

          // После подтверждения залога покупатель получает и обычное уведомление о статусе заказа.
          if (isConfirm) {
            try {
              const telegramId = normalizeTelegramId(responseOrder.telegram_user_id);
              if (telegramId) await sendCustomerOrderStatus(responseOrder, telegramId);
            } catch (notificationError) {
              console.error("Customer confirmed order notification:", notificationError.message);
            }
          }

          await answerCallbackQuery(
            callback.id,
            isConfirm ? "Заказ подтверждён" : "Залог отклонён"
          );
          return;
        }
