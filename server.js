...
app.post(
  ...
  async (
    ...
    try {
      ...
      /*
       * Turso/libSQL can return lastInsertRowid in different
       * numeric representations. Resolve the created order
       * defensively and never access .id on null.
       */
      const insertedId = Number(result?.lastInsertRowid);
      let order = null;

      if (Number.isInteger(insertedId) && insertedId > 0) {
        order = await getOrderById(insertedId);
      }

      /* Fallback: find the newest order of this Telegram user. */
      if (!order) {
        ...
        order = fallback.rows[0] || null;
      }

      if (!order) {
        throw new Error("Заказ создан, но сервер не смог получить его ID");
      }
