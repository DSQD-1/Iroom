...
function validatePromoCode(
  ...
  if (
    ...
    return promoError(
      `Минимальная сумма заказа — ${formatPrice(
        minOrder
      )}`
    );
  }
  ...
  if (
    ...
    return promoError(
      "Промокод имеет некорректную скидку"
    );
  }
...
async function notifyAdminsAboutOrder(
  ...
  const promoBlock =
    order.promo_code
      ? `
<b>Промокод:</b> ${escapeTelegramHtml(
          order.promo_code
        )}

<b>Скидка:</b> −${formatPrice(
...
app.post(
  ...
  async (
    ...
    try {
       ...
       * ПРОМОКОД.
       *
       * Цена рассчитывается сервером.
       * Значение final_amount от клиента
       * НЕ используется.
       */
      const promoCode =
        normalizePromoCode(
      ...
      if (promoCode) {
        ...
        /*
         * Атомарно резервируем одно
         * использование промокода.
         *
         * Учитываем срок действия ещё раз
      ...
      try {
        ...
        /*
         * Если заказ не создался после
         * резервирования промокода,
         * возвращаем использование.
         */
        if (
          promoReserved &&
          reservedPromoCode
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            error:
              "Промокод может содержать только латинские буквы, цифры, _ и -"
          });
      }
